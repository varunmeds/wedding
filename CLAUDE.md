...
`index.html` carries Open Graph and Twitter card tags. They contain a `https://example.com` placeholder that **must be replaced with the real domain** before the link is shared, because WhatsApp and most scrapers ignore relative `og:image` URLs.
